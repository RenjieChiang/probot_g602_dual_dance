...
\n\
# rosservice probot_control_command\n\
CMD_MOTION_SIMULATION              = 1\n\
CMD_MOTION_LIVE                    = 2\n\
CMD_MOTION_ENABLE                  = 3\n\
CMD_MOTION_DISABLE                 = 4\n\
CMD_MOTION_STOP                    = 5\n\
CMD_ERROR_CLEAR                    = 6\n\
CMD_DRAG_TEACHING_START            = 7\n\
CMD_DRAG_TEACHING_STOP             = 8\n\
CMD_ZERO_CALIBRATION_AUTO          = 9\n\
CMD_ZERO_CALIBRATION_MANUAL        = 10\n\
CMD_UPDATE_ZERO_PLUSE_CNT          = 11\n\
\n\
# rosservice probot_digital_output\n\
TYPE_DO_CONTROLLER            = 1\n\
TYPE_DO_CONTROLLER_EXPAND     = 2\n\
TYPE_DO_END_CONNECTOR         = 3\n\
TYPE_DO_OTHERS                = 4\n\
\n\
IO_OFF  = 0\n\
IO_ON   = 1\n\
\n\
# rosservice probot_electric_gripper\n\
MASK_POSITION = 2\n\
MASK_VELOCITY = 4\n\
\n\
control_mode = 0\n\
...
try:\n\
  ...
  var dropdown_arm = block.getFieldValue('arm');
  if (dropdown_arm == 0) {
    code += "xarm5";
  } else if (dropdown_arm == 1) {
    code += "xarm6";
  } else if (dropdown_arm == 2) {
    code += "xarm7";
  } else {
    // For robcell controller
    code += "manipulator";
  }
...
def get_digital_input(io_num):\n\
  if probot_initialized != 1:\n\
    ...
    sys.exit()\n\
  if control_mode != CMD_MOTION_LIVE:\n\
    rospy.logerr(\"Run program failed! IO Blocks shoul be used in Live mode.\")\n\
    return -1\n\
  if io_num > 8:\n\
    rospy.logerr(\"Run program failed! The IO number specified is out of range(1-8).\")\n\
    return -1\n\
\n\
  m_pipe = subprocess.Popen(\"rostopic echo -n 1 probot_status | grep -w digitalInput\", shell=True, stdout=subprocess.PIPE)\n\
  m_pipe_out = m_pipe.stdout.readlines()\n\
  # line format:\n\
  # digitalInput: [False, True, False, False, False, False, False, False, False, False, False, False, False, False, False, False]\n\
  for line in m_pipe_out:\n\
    input_val_str = line.replace(\"False\", \"0\").replace(\"True\", \"1\").strip()\n\
    io_level = input_val_str[15+3*(io_num-1)]\n\
...
";
  ...
  if (dropdown_mode == 0) {
    code += "\n\
control_mode = CMD_MOTION_LIVE\n\
shell_cmd = \"rosservice call probot_control_command \" + str(CMD_MOTION_LIVE) + \" >/dev/null 2>&1\"\n\
";
  } else {
    code += "\n\
control_mode = CMD_MOTION_SIMULATION\n\
shell_cmd = \"rosservice call probot_control_command \" + str(CMD_MOTION_SIMULATION) + \" >/dev/null 2>&1\"\n\
";
  }
  ...
  if (checkbox_enable) {
    code += "\n\
if control_mode == CMD_MOTION_LIVE:\n\
  shell_cmd = \"rosservice call probot_control_command \" + str(CMD_MOTION_ENABLE) + \" >/dev/null 2>&1\"\n\
  os.system(shell_cmd)\n\
";
  }
...
# 机械臂停止运动\n\
if control_mode == CMD_MOTION_LIVE:\n\
  shell_cmd = \"rosservice call probot_control_command \" + str(CMD_MOTION_STOP) + \" >/dev/null 2>&1\"\n\
  os.system(shell_cmd)\n\
";
  }
  if (checkbox_gohome) {
...
# 除能机械臂控制\n\
if control_mode == CMD_MOTION_LIVE:\n\
  shell_cmd = \"rosservice call probot_control_command \" + str(CMD_MOTION_DISABLE) + \" >/dev/null 2>&1\"\n\
  os.system(shell_cmd)\n\
";
  }
  code += "\n\
...
# 设置控制器数字IO输出电平\n\
io_mask = 1 << (io_num - 1)\n\
shell_cmd = \"rosservice call probot_digital_output \" + str(io_type) + \" \" + str(io_mask) + \" \" + str(io_status) + \" >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n\
";
  return code;
};
...
Blockly.Python['set_electric_gripper'] = function(block) {
  var text_velocity_scale = block.getFieldValue('velocity_scale');
  var text_position_scale = block.getFieldValue('position_scale');

  var code = "\n\
# 控制xarm电动夹爪，必须先设置速度\n\
shell_cmd = \"rosservice call probot_electric_gripper \" + str(MASK_VELOCITY) + \" " + text_position_scale + " " + text_velocity_scale + " >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n\
shell_cmd = \"rosservice call probot_electric_gripper \" + str(MASK_POSITION) + \" " + text_position_scale + " " + text_velocity_scale + " >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n";
  return code;
};

Blockly.Python['set_vaccum_gripper'] = function(block) {
  var dropdown_enable = block.getFieldValue('enable');
  var text_pump_io_num = block.getFieldValue('pump_io_num');
  var text_changer_io_num = block.getFieldValue('changer_io_num');
  var text_time_sleep_s = block.getFieldValue('time_sleep_s');

  var code = "";
  if (dropdown_enable == 0) {
    code = code + "\n\
# 打开气泵\n\
shell_cmd = \"rosservice call probot_vaccum_gripper True " + text_pump_io_num + " " + text_pump_io_num + " >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n\
rospy.sleep(" + text_time_sleep_s + ")\n\
# 打开气阀\n\
shell_cmd = \"rosservice call probot_vaccum_gripper True " + text_changer_io_num + " " + text_changer_io_num + " >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n\
";
  } else {
    code = code + "\n\
# 关闭气泵\n\
shell_cmd = \"rosservice call probot_vaccum_gripper False " + text_pump_io_num + " " + text_pump_io_num + " >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n\
rospy.sleep(" + text_time_sleep_s + ")\n\
# 关闭气阀\n\
shell_cmd = \"rosservice call probot_vaccum_gripper False " + text_changer_io_num + " " + text_changer_io_num + " >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n\
";
  }
  return code;
