...
var ExecutionLogicModule = (function () {
  ...
  return {
    ...
    save_python_code: function(code) {
        var message_data = 'save_python_code\n' + code;
        socket.send(message_data);
    },
    ...
    // 基于ros rviz webkit和QT5.9 webengine，以下函数均无法打开文件保存窗口
    // save_blockly_code: function() {
    ...
    // },
    // 改成将文本发送至probot_blockly_backend.py后端处理
    save_blockly_code: function() {
      var filename = 'blockly_workspace.xml';
      var xml = Blockly.Xml.workspaceToDom(workspace);
      var xml_text = Blockly.Xml.domToText(xml);
      if (xml_text.length > 0) {
        var message_data = 'save_blockly_code\n' + xml_text;
        socket.send(message_data);
      }
    },
